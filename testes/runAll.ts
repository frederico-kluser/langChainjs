...
// Função para executar cada LLM em paralelo
async function runAllLLMs() {
  console.log('🤖 Testando todos os modelos LLM em paralelo 🤖\n');
  
  const prompt = 'Responda com menos de 50 caracteres: Qual é a capital do Brasil?';
  ...
  const results = await Promise.allSettled(
    modelConfigs.map(async (config) => {
      const spinner = ora({
        text: `Executando ${config.name}...`,
        color: 'blue'
      }).start();
      
      try {
        const startTime = Date.now();
        const response = await getAIResponse(prompt, { model: config.model });
  ...
  try {
    ...
    const filePath = path.join(resultsDir, `resultado-teste-${timestamp}.json`);
